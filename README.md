...
*   **智能场景识别**：
    *   **应用窗口监听**：离开 IDE 时自动切换至中文输入法，返回时按当前编辑器上下文恢复输入法状态。
...
### v1.5.1
*   🛠️ **修复 `ALT+TAB` 误触发切换**：避免在 IDEA 窗口来回切换时误按中英文切换热键。
*   🔔 **修复错误失败提示**：避免窗口重新激活阶段因焦点未稳定而弹出“切换到英文失败”提醒。
*   ⚙️ **优化输入法恢复策略**：IDE 重新激活后按真实编辑器上下文恢复输入法，不再一律强制切回英文。
